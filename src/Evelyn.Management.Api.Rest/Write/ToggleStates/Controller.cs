namespace Evelyn.Management.Api.Rest.Write.ToggleStates
{
    ...
    [Route("api/projects/{projectId}/environments/{environmentKey}/toggles/{toggleId}")]
    ...
    public class Controller : EvelynController
    {
        ...
        [HttpPost]
        public async Task<ObjectResult> Post(Guid projectId, string environmentKey, Guid toggleId, [FromBody]Messages.ChangeToggleState message)
        {
            // TODO: validation
            try
            {
                var command = new Core.WriteModel.Commands.ChangeToggleState(UserId, projectId, environmentKey, toggleId, message.State, message.ExpectedVersion);
